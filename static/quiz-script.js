...
(function () {
  ...
  function getCorrectAnswerText(q) {
    ...
    if (isAnyFillInTheBlank(q) && Array.isArray(q.correct)) {
      ...
      console.log('[Quiz] getCorrectAnswerText for FITB:', {
        type: q.type,
        blanksCount,
        correct: q.correct
      });
      ...
      for (let i = 0; i < blanksCount; i++) {
        let acceptableAnswers = q.correct[i];
        
        // Ensure acceptableAnswers is an array
        if (!Array.isArray(acceptableAnswers)) {
          acceptableAnswers = acceptableAnswers ? [acceptableAnswers] : [];
        }
        ...
        if (acceptableAnswers.length === 1) {
          ...
          answers.push('(no answer defined)');
        }
      }
      
      console.log('[Quiz] FITB answers array:', answers);
  ...
  function checkFillInTheBlankAnswer(q, userAnswers) {
    if (!Array.isArray(q.correct)) {
      console.log('[Quiz] checkFillInTheBlankAnswer: q.correct is not an array');
    ...
    console.log('[Quiz] checkFillInTheBlankAnswer:', { blanksCount, correct: q.correct, userAnswers });
    ...
    for (let i = 0; i < blanksCount; i++) {
      ...
      console.log('[Quiz] Checking blank', i, ':', { userAnswer, acceptableAnswers, isCorrect });
  ...
  function showFeedback(isCorrect, q) {
    console.log('[Quiz] showFeedback called:', { isCorrect, questionType: q.type, hasRationale: !!q.rationale });
    
    if (!els.feedback) {
      console.log('[Quiz] els.feedback not found!');
      return;
    }
    ...
    console.log('[Quiz] correctAnswer:', correctAnswer);
    ...
    if (els.answerLine && correctAnswer) {
      ...
      console.log('[Quiz] answerLine updated');
    } else {
      console.log('[Quiz] answerLine skipped:', { hasElement: !!els.answerLine, hasAnswer: !!correctAnswer });
    }
    ...
    if (els.rationale && q.rationale) {
      ...
      console.log('[Quiz] rationale updated');
    } else {
      console.log('[Quiz] rationale skipped:', { hasElement: !!els.rationale, hasRationale: !!q.rationale });
    }
  }
