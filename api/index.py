...
CFRN_DOMAIN_FILES = {
    ...
    'Resuscitation Principles':
        'CFRN_Resuscitation_Principles.json',
    'Trauma':
        'CFRN_Trauma.json',
    'Medical Emergencies':
        'CFRN_Medical_Emergencies.json',
    'Special Populations':
...
def _load_questions_from_file(path):
    ...
    try:
        ...
        print(f"[_load_questions_from_file] Error reading {path}: {e}")
        return []
...
def load_cfrn_domain_questions(domain_name):
    ...
    if filename:
        ...
        if path.exists():
            ...
            print(f"[CFRN] Loaded {len(questions)} questions from {filename}")
            return questions
        else:
    ...
    print(f"[CFRN] Fallback: {len(filtered)} questions for domain '{domain_name}'")
    return filtered
...
def load_cfrn_questions():
    ...
    for domain, filename in CFRN_DOMAIN_FILES.items():
        path = MODULES_DIR / 'Nursing_Certifications' / filename
        if not path.exists():
            print(f"[CFRN] Domain file not found, skipping: {filename}")
            continue
        questions = _load_questions_from_file(path)
        all_questions.extend(questions)
        print(f"[CFRN] Merged {len(questions)} questions from {filename}")

    if not all_questions:
        print("[CFRN] No domain files found - falling back to CFRN_Question_Bank.json")
        all_questions = _load_cfrn_legacy()

    print(f"[CFRN] Total questions loaded: {len(all_questions)}")
...
def paper_prompt_builder():
    try:
        return render_template('paper-prompt-builder.html')
    except Exception as e:
...
def cfrn_domain_quiz(domain_name):
    try:
        ...
        filtered_questions = load_cfrn_domain_questions(domain_name)
