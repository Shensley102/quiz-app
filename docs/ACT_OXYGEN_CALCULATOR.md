...
**Estimated Oxygen Availability** applies the configured device consumption to every entered source and displays independent on-board and portable runtimes. These runtimes are comparisons at the same settings; they are not added together and do not imply automatic source switching. An incomplete source is identified without hiding the estimate for another valid source. The transport planner remains the place to model sequential source changes by phase.
